  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.2 }} className="flex flex-col h-full">
      <div className="relative aspect-square p-2">
        ...
        <div className="absolute top-3 right-3 bg-slate-900/90 backdrop-blur-xl border border-slate-700/50 rounded-full px-2 py-0.5 flex items-center gap-1.5"><div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div><span className="text-[10px] text-white font-bold" style={{ fontFamily: 'Orbitron, monospace' }}>Actif</span></div>
      </div>
      <div className="p-3 flex-grow flex flex-col">
        <h3 className="text-lg font-bold text-white mb-1" style={{ fontFamily: 'Orbitron, monospace' }}>{member.name}</h3>
        <p className="text-cyan-400 font-bold mb-2 text-sm" style={{ fontFamily: 'Rajdhani, sans-serif' }}>{member.role}</p>
        <p className="text-slate-300 text-sm leading-relaxed mb-4 flex-grow" style={{ fontFamily: 'Rajdhani, sans-serif' }}>{member.bio}</p>
        
        <div className="flex gap-2 mb-4">
          <a href={member.portfolioUrl} target="_blank" rel="noopener noreferrer" className="p-2 bg-slate-800/50 border border-slate-700/50 rounded-lg text-slate-400 hover:text-cyan-400 hover:border-cyan-500/50 transition-all"><ExternalLink className="w-4 h-4" /></a>
          {member.socials.linkedin && <a href={member.socials.linkedin} target="_blank" rel="noopener noreferrer" className="p-2 bg-slate-800/50 border border-slate-700/50 rounded-lg text-slate-400 hover:text-blue-400 hover:border-blue-500/50 transition-all"><Linkedin className="w-4 h-4" /></a>}
          {member.socials.github && <a href={member.socials.github} target="_blank" rel="noopener noreferrer" className="p-2 bg-slate-800/50 border border-slate-700/50 rounded-lg text-slate-400 hover:text-white hover:border-slate-500/50 transition-all"><Github className="w-4 h-4" /></a>}
          {member.socials.facebook && <a href={member.socials.facebook} target="_blank" rel="noopener noreferrer" className="p-2 bg-slate-800/50 border border-slate-700/50 rounded-lg text-slate-400 hover:text-blue-500 hover:border-blue-500/50 transition-all"><Facebook className="w-4 h-4" /></a>}
        </div>
        
        <div className="space-y-2 mt-auto">
          <a href={member.cvUrl} download className="block w-full py-2.5 bg-slate-800/50 border border-slate-700/50 text-slate-300 font-bold rounded-lg hover:bg-slate-700/50 hover:border-cyan-500/50 transition-all text-center text-sm" style={{ fontFamily: 'Orbitron, monospace' }}><span className="flex items-center justify-center gap-2"><Download className="w-3.5 h-3.5" />Download CV</span></a>
          {!isLastMember ? (
            <button onClick={onNext} className={`w-full py-2.5 bg-gradient-to-r ${member.gradient} text-white font-bold rounded-lg hover:shadow-lg transition-all text-center text-sm`} style={{ fontFamily: 'Orbitron, monospace' }}><span className="flex items-center justify-center gap-2">Membre Suivant<ChevronRight className="w-4 h-4" /></span></button>
          ) : (
...
export default function Team() {
  ...
  return (
    <section id="equipe" className="py-12 sm:py-20 relative bg-slate-950 overflow-hidden">
      <div className="absolute inset-0"><div className="absolute top-1/4 left-1/4 w-96 h-96 bg-cyan-500/5 rounded-full blur-3xl animate-pulse"></div><div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-purple-500/5 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }}></div></div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-14">
          <div className="inline-flex items-center gap-2 bg-slate-800/50 backdrop-blur-xl border border-cyan-500/30 rounded-full px-5 py-2.5 mb-5"><Terminal className="w-4 h-4 text-cyan-400" /><span className="text-sm text-cyan-400 font-medium" style={{ fontFamily: 'Orbitron, monospace' }}>$ team --list-members</span></div>
          <h2 className="text-5xl font-bold mb-5" style={{ fontFamily: 'Orbitron, monospace' }}>L'<span className="bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-500 bg-clip-text text-transparent">Équipe</span></h2>
          <p className="text-lg text-slate-400 max-w-3xl mx-auto leading-relaxed" style={{ fontFamily: 'Rajdhani, sans-serif' }}>Rencontrez les talents qui composent notre équipe de développement. Des passionnés qui transforment les idées en réalité digitale.</p>
        </div>
        ...
        <div className="text-center mt-8">
          <button onClick={openTeamView} aria-label="Voir l'équipe en action" className="group relative px-7 py-3.5 bg-gradient-to-r from-cyan-500 to-blue-500 text-white font-bold rounded-full overflow-hidden shadow-lg hover:shadow-cyan-500/50 transition-all duration-300 hover:scale-105 text-base" style={{ fontFamily: 'Orbitron, monospace' }}>
            ...
            <span className="relative flex items-center gap-2"><Users className="w-5 h-5"/> View Team</span>
          </button>
        </div>
      </div>
        ...
        {activeMember && (
          <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm p-4 pt-20 sm:pt-24 flex items-center justify-center">
            <motion.div 
              ...
              className="w-full max-w-md h-full max-h-[85vh] bg-slate-900/80 backdrop-blur-xl border border-cyan-500/30 rounded-lg shadow-2xl shadow-cyan-500/10 flex flex-col"
            >
              <div className="flex-shrink-0 bg-slate-800/80 rounded-t-lg py-2 px-4 flex items-center justify-between border-b border-slate-700">
                ...
                <p className="text-sm text-slate-400" style={{ fontFamily: 'monospace' }}>/home/team/{activeMember.name.split(' ')[0].toLowerCase()}</p>
                <button onClick={closeTerminals} className="p-1.5 -m-1.5 text-slate-400 hover:text-white hover:bg-slate-700/80 rounded-md transition-colors"><X size={18} /></button>
              </div>
              <div className="flex-grow p-4 overflow-y-auto">
                {terminalState !== TERMINAL_STATES.SHOWING_MEMBER ? (
                  ...
                  <TeamMemberDetailView member={activeMember} onNext={goToNextMember} isLastMember={activeMemberIndex === teamMembers.length - 1} onClose={closeTerminals} />
                )}
              </div>
            </motion.div>
          </div>
